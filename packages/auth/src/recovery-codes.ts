...
const stripFormat = (input: string): string =>
...
export const redeemRecoveryCode = async (
  ...
  const { results } = await db.prepare(
    'SELECT salt_hex, code_hash FROM recovery_codes WHERE user_id = ? AND used_at IS NULL'
  ).bind(userId).all<Row>()
  for (const row of results) {
    ...
    if (candidateHash === row.code_hash) {
      ...
      ).bind(now, userId, row.code_hash).run()
      return { ok: true }
    }
  }
  return { ok: false }
...
export const countUnusedCodes = async (
  db: D1Database,
  userId: string
): Promise<number> => {
