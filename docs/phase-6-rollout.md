...
- [ ] `@undeadliner/pya-*` packages live on the chosen registry (see [`publishing.md`](./publishing.md)).
...
### 1. `@undeadliner/pya-shared` swap

The schemas are identical to `@pyaeats/shared`. Add `@undeadliner/pya-shared` as a dep alongside `@pyaeats/shared`, then sed-rename the imports file by file. Type-checker catches misses.

- Files touched: ~40 across `apps/api`, `apps/site`, `apps/admin`.
- Risk: schemas drift between the two shared packages mid-rollout.
- Mitigation: keep `@pyaeats/shared` re-exporting from `@undeadliner/pya-shared` during the swap; delete only after the last consumer is migrated.

### 2. `@undeadliner/pya-audit` + `@undeadliner/pya-email`
...
### 3. `@undeadliner/pya-ui` + `@undeadliner/pya-tokens` swap

Replace `import '@pyaeats/tokens/index.css'` with `import '@undeadliner/pya-tokens/index.css'` and `from '@pyaeats/ui'` with `from '@undeadliner/pya-ui'`. CSS variables are byte-identical between the two; visual regression should be zero.

- Visual smoke test: load each route in the prod-built site and diff against pre-cutover screenshots.

### 4. `@undeadliner/pya-auth` cutover (the big one)

`apps/api/src/features/auth/*` already lives in `@undeadliner/pya-auth`. The refactor:

```ts
// apps/api/src/index.ts
import { passwordlessRoutes, oauthRoutes, createDevBypassRoutes, requireAuth } from '@undeadliner/pya-auth'
...
### 5. Lift real code into `@undeadliner/pya-{cf,reviews,comments,cms}`
...
3. Replace the pyaeats-app feature dir with a thin re-export from `@undeadliner/pya-<feature>`.
...
| Step | Time | Risk |
|---|---|---|
| 1 — `@undeadliner/pya-shared` | 1 h | low |
| 2 — `@undeadliner/pya-audit` + `@undeadliner/pya-email` | 1 h | low |
| 3 — `@undeadliner/pya-ui` + `@undeadliner/pya-tokens` | 1 h | low-medium (visual regression) |
| 4 — `@undeadliner/pya-auth` | 3-4 h | **high** — full preview e2e first |
| 5 — `@undeadliner/pya-{cf,reviews,comments,cms}` | 4 h | medium |
