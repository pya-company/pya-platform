...
- [ ] `@pya-company/*` packages live on the chosen registry (see [`publishing.md`](./publishing.md)).
...
### 1. `@pya-company/shared` swap

The schemas are identical to `@pyaeats/shared`. Add `@pya-company/shared` as a dep alongside `@pyaeats/shared`, then sed-rename the imports file by file. Type-checker catches misses.

- Files touched: ~40 across `apps/api`, `apps/site`, `apps/admin`.
- Risk: schemas drift between the two shared packages mid-rollout.
- Mitigation: keep `@pyaeats/shared` re-exporting from `@pya-company/shared` during the swap; delete only after the last consumer is migrated.

### 2. `@pya-company/audit` + `@pya-company/email`
...
### 3. `@pya-company/ui` + `@pya-company/tokens` swap

Replace `import '@pyaeats/tokens/index.css'` with `import '@pya-company/tokens/index.css'` and `from '@pyaeats/ui'` with `from '@pya-company/ui'`. CSS variables are byte-identical between the two; visual regression should be zero.

- Visual smoke test: load each route in the prod-built site and diff against pre-cutover screenshots.

### 4. `@pya-company/auth` cutover (the big one)

`apps/api/src/features/auth/*` already lives in `@pya-company/auth`. The refactor:

```ts
// apps/api/src/index.ts
import { passwordlessRoutes, oauthRoutes, createDevBypassRoutes, requireAuth } from '@pya-company/auth'
...
### 5. Lift real code into `@pya-company/{cf,reviews,comments,cms}`
...
3. Replace the pyaeats-app feature dir with a thin re-export from `@pya-company/<feature>`.
...
| Step | Time | Risk |
|---|---|---|
| 1 — `@pya-company/shared` | 1 h | low |
| 2 — `@pya-company/audit` + `@pya-company/email` | 1 h | low |
| 3 — `@pya-company/ui` + `@pya-company/tokens` | 1 h | low-medium (visual regression) |
| 4 — `@pya-company/auth` | 3-4 h | **high** — full preview e2e first |
| 5 — `@pya-company/{cf,reviews,comments,cms}` | 4 h | medium |
