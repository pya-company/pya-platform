...
- [ ] `@pya-platform/*` packages live on the chosen registry (see [`publishing.md`](./publishing.md)).
...
### 1. `@pya-platform/shared` swap

The schemas are identical to `@pyaeats/shared`. Add `@pya-platform/shared` as a dep alongside `@pyaeats/shared`, then sed-rename the imports file by file. Type-checker catches misses.

- Files touched: ~40 across `apps/api`, `apps/site`, `apps/admin`.
- Risk: schemas drift between the two shared packages mid-rollout.
- Mitigation: keep `@pyaeats/shared` re-exporting from `@pya-platform/shared` during the swap; delete only after the last consumer is migrated.

### 2. `@pya-platform/audit` + `@pya-platform/email`
...
### 3. `@pya-platform/ui` + `@pya-platform/tokens` swap

Replace `import '@pyaeats/tokens/index.css'` with `import '@pya-platform/tokens/index.css'` and `from '@pyaeats/ui'` with `from '@pya-platform/ui'`. CSS variables are byte-identical between the two; visual regression should be zero.

- Visual smoke test: load each route in the prod-built site and diff against pre-cutover screenshots.

### 4. `@pya-platform/auth` cutover (the big one)

`apps/api/src/features/auth/*` already lives in `@pya-platform/auth`. The refactor:

```ts
// apps/api/src/index.ts
import { passwordlessRoutes, oauthRoutes, createDevBypassRoutes, requireAuth } from '@pya-platform/auth'
...
### 5. Lift real code into `@pya-platform/{cf,reviews,comments,cms}`
...
3. Replace the pyaeats-app feature dir with a thin re-export from `@pya-platform/<feature>`.
...
| Step | Time | Risk |
|---|---|---|
| 1 — `@pya-platform/shared` | 1 h | low |
| 2 — `@pya-platform/audit` + `@pya-platform/email` | 1 h | low |
| 3 — `@pya-platform/ui` + `@pya-platform/tokens` | 1 h | low-medium (visual regression) |
| 4 — `@pya-platform/auth` | 3-4 h | **high** — full preview e2e first |
| 5 — `@pya-platform/{cf,reviews,comments,cms}` | 4 h | medium |
